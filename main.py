...
4. "EXECUTE": Provide the final, clean, production-ready code.
 ...
 i forced the LLM to send use the response exactly with the json format

 - You must strictly output in the specified JSON format.

 after that i want to tell our model to return just one step in each API call

 - Generate only ONE state transition per API call.

and after each step i want to send that again to the LLM to send me the next step because
LLM API calls are stateless. The model does not remember this analysis unless we manually feed it back into the history.
